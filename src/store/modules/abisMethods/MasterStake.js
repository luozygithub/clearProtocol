...
function judgeToken(rootState) {
    if (!state.token) state.token = getContract.getContractByName('MasterStake', rootState.app.web3)
}
...
const actions = {
    ...
    setStake({rootState}, _addr) {
        judgeToken(rootState)
        return new Promise((resolve, reject) => {
            state.token.methods.setStake(_addr).estimateGas({
                from: rootState.app.account,
            }).then(gas => {
                state.token.methods.setStake(_addr).send({
                    from: rootState.app.account,
                    gas: parseInt(gas * 1.2)
                }).then(res => {
                    let operateLogs = localStorage.getItem("operateLogs") ? JSON.parse(localStorage.getItem("operateLogs")) : []
    ...
    stake({rootState}, {value, _usr}) {
        judgeToken(rootState)
        return new Promise((resolve, reject) => {
            state.token.methods.stake(_usr).estimateGas({
                from: rootState.app.account,
                value: rootState.app.web3.utils.toWei(value.toString())
            }).then(gas => {
                state.token.methods.stake(_usr).send({
                    from: rootState.app.account,
                    gas: parseInt(gas * 1.2),
                    value: rootState.app.web3.utils.toWei(value.toString())
                }).then(res => {
    ...
    stakeExt({rootState}) {
        judgeToken(rootState)
        return new Promise((resolve, reject) => {
            state.token.methods.stakeExt().call().then(res => {
                resolve(res)
            }).catch(err => {
                      reject(JSON.parse(err.message.substr(24,err.message.length)).message)
            })
