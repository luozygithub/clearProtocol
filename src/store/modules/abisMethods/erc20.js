...
function judgeToken(rootState,address) {
    if (!state.token) state.token = getContract.getContractByToken('erc20', address,rootState.app.web3)
}

const state = {};
const mutations = {};
const actions = {
    allowance({rootState}, {owner, spender,address}) {
    ...
    approve({rootState}, {spender, amount,address}) {
        judgeToken(rootState,address)
        return new Promise((resolve, reject) => {
